...
def Pricing():
    ...
    with upper_panel:
        ...
        with col1:
            ...
            st.markdown(
                '<style>.stSelectbox {height: 30px; box-shadow: 0px 5px 12px #ffffff;border-radius: 2px;background:#ffffff;</style>',
        ...
        final_df = RESULT.loc[RESULT["CATEGORY"].isin([category_selection])]
        final_df = RESULT.loc[RESULT["CATEGORY"] == category_selection]
        ...
        fig = alt.Chart(pd_df_lsrr_pivot).mark_bar().encode(
            x=alt.X('STOCK_NAME:N', title='STOCK NAME', axis=alt.Axis(labelAngle=-80, labelFontSize=8, titleFontWeight='normal')),  # Change font weight here
            y=alt.Y('Price:Q', title='PRICE', axis=alt.Axis(titleFontWeight='normal')),  # Change font weight here
